...
export function appendMeteringSample(samples: number[], metering: number) {
  ...
  return nextSamples.length <= MaxMeteringSampleCount
    ? nextSamples
    : nextSamples.slice(nextSamples.length - MaxMeteringSampleCount);
}

export function createWaveformPeaksFromMetering(samples: number[], peakCount = DefaultWaveformPeakCount) {
  ...
  const bucketCount = Math.min(MaxStoredWaveformPeakCount, Math.max(1, Math.floor(peakCount)));
  ...
  return Array.from({ length: bucketCount }, (_, bucketIndex) => {
    ...
    const bucketLevels = finiteSamples.slice(startIndex, endIndex).map(normalizeMeteringDb);
    return roundPeak(getRepresentativePeak(bucketLevels));
  });
}

export function createDisplayWaveformPeaks(value: unknown, maxPeakCount = DefaultWaveformPeakCount) {
  const targetPeakCount = Math.min(MaxStoredWaveformPeakCount, Math.max(1, Math.floor(maxPeakCount)));
